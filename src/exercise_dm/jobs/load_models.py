...
def model_loader(model_name, context_mode):
    ...
    if context_mode == "code":
        if model_name == "sbert_st":
            model = torch.load(FINETUNED_MODEL_SBERT_ST_CODE, map_location=torch.device('mps'))
        elif model_name == "sbert_ce":
            model = torch.load(FINETUNED_MODEL_SBERT_CE_CODE, map_location=torch.device('mps'))
        elif model_name == "concat_bert":
            model = torch.load(FINETUNED_MODEL_CONCAT_BERT_CODE, map_location=torch.device('mps'))
        elif model_name == "gpt2":
            model = torch.load(FINETUNED_MODEL_GPT2_CODE, map_location=torch.device('mps'))
    elif context_mode == "text":
        if model_name == "sbert_st":
            model = torch.load(FINETUNED_MODEL_SBERT_ST_TEXT, map_location=torch.device('mps'))
        elif model_name == "sbert_ce":
            model = torch.load(FINETUNED_MODEL_SBERT_CE_TEXT, map_location=torch.device('mps'))
        elif model_name == "concat_bert":
            model = torch.load(FINETUNED_MODEL_CONCAT_BERT_TEXT, map_location=torch.device('mps'))
        elif model_name == "gpt2":
            model = torch.load(FINETUNED_MODEL_GPT2_TEXT, map_location=torch.device('mps'))
    return model