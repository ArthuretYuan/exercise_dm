import torch
import numpy as np
from exercise_dm.core.training.train_concat_bert import DistilBertConcatMultiPassageClassifier
from transformers import BertTokenizer

# Load the model
tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
model = torch.load('model/concat_bert/concat_bert_model.pt')
print("Concat BERT model loaded successfully")
device = "mps"
model.eval()
model = model.to(device)

def predict_with_concat_bert(text1, text2, context):
